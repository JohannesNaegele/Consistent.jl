...
aggr = model(
    ...
    eqs=@equations begin
        ...
        RRₗ = ((1 + rₗ) / (1 + π)) - 1 # 11.9: Real interest rate on loans
...
firms₄ = model(
    ...
    eqs=@equations begin
        ...
        Eks = Eks[-1] + ((1 - ψᵤ) * INV[-1]) / Pₑ  # 11.41: Supply of equities issued by firms
...
hh₁ = model(
    ...
    eqs=@equations begin
        ...
        YDkre = ε * YDkr + (1 - ε) * (YDkr[-1] * (1 + GRpr)) # 11.54: Expected real regular disposable income
...
banks₁ = model(
    ...
    eqs=@equations begin
        ...
        z2a = (BLR[-1] > (top + 0.05)) ? 1 : 0
        z2b = (BLR[-1] > top) ? 1 : 0
        z1a = (BLR[-1] ≤ bot) ? 1 : 0
        z1b = (BLR[-1] ≤ (bot - 0.05)) ? 1 : 0
    end
)
...
banks₂ = model(
    ...
    eqs=@equations begin
        ...
        ADDl = (Fbt - rb[-1] * Bbd[-1] + rₘ[-1] * (Ms[-1] - (1 - NPLke) * Lfs[-1] - Lhs[-1])) / ((1 - NPLke) * Lfs[-1] + Lhs[-1])
